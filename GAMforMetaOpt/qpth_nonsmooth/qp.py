        ...
        def backward(ctx, dl_dzhat):
            ...
            for j in range(zhats_15.shape[0]):
                ...
                for i in range(zhats_15[j].shape[0]):
                    ...
                    if abs(a)+abs(b)<tor and abs(a)+abs(b)<value:
                        value=abs(a)+abs(b)
                        boundary=i
            ...
            lams1= ctx.lams.clone()
            lams2= ctx.lams.clone()
            slacks1=ctx.slacks.clone()
            slacks2=ctx.slacks.clone()

            for j in range(zhats.shape[0]):
                if boundary_list[j]!=None:
                    lams1.data[j,boundary_list[j]]=0
                    #slacks1.data[j,boundary_list[j]]=-0.1
                    #lams2.data[j,boundary_list[j]]=0.1
                    slacks2.data[j,boundary_list[j]]=0
            ...
            d1 = torch.clamp(lams1, min=1e-8) / torch.clamp(slacks1, min=1e-8)
            ...
            dx1, _, dlam1, dnu1 = pdipm_b.solve_kkt(
                ...
                torch.zeros(nBatch, neq).type_as(G) if neq > 0 else torch.Tensor())
            dps1 = dx1
            dGs1 = bger(dlam1, zhats) + bger(lams1, dx1)
            if G_e:
                dGs1 = dGs1.mean(0)
            dhs1 = -dlam1
            if h_e:
            ...
            d2 = torch.clamp(lams2, min=1e-8) / torch.clamp(slacks2, min=1e-8)
            ...
            dx2, _, dlam2, dnu2 = pdipm_b.solve_kkt(
                ...
                torch.zeros(nBatch, neq).type_as(G) if neq > 0 else torch.Tensor())
            dps2 = dx2
            dGs2 = bger(dlam2, zhats) + bger(lams2, dx2)
            if G_e:
                dGs2 = dGs2.mean(0)
            dhs2 = -dlam2
            if h_e:
            ...
            dQs_final=torch.zeros_like(dQs1)
            for j in range(zhats.shape[0]):
                c=torch.dot(torch.reshape(dQs2.data[j],(-1,)),torch.reshape(dQs1.data[j],(-1,)))
                a1=torch.norm(dQs2.data[j])*torch.norm(dQs2.data[j])-c+1e-8
                a2=torch.norm(dQs1.data[j])*torch.norm(dQs1.data[j])-c+1e-8
                b=torch.norm(dQs2.data[j]-dQs1.data[j])*torch.norm(dQs2.data[j]-dQs1.data[j])+2e-8
                if a1/b>1.0:
                    alpha=1.0
                elif a1/b<0.0:
                    alpha=0.0
                ...
                if a2/b>1.0:
                    ...
                    beta=a2/b
                print(alpha,beta)
                dQs_final.data[j]=alpha*dQs1[j]+beta*dQs2[j]

            grads=(dQs_final, 0.5*dps1+0.5*dps2, 0.5*dGs1+0.5*dGs2, 0.5*dhs1+0.5*dhs2, 0.5*dAs1+0.5*dAs2, 0.5*dbs1+0.5*dbs2)
            #grads=(0.5*dQs1+0.5*dQs2, 0.5*dps1+0.5*dps2, 0.5*dGs1+0.5*dGs2, 0.5*dhs1+0.5*dhs2, 0.5*dAs1+0.5*dAs2, 0.5*dbs1+0.5*dbs2)
            return grads
