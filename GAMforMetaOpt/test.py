...
if __name__ == '__main__':
    ...
    log_file_path = opt.load+ "/test_log.txt"
    ...
    for j in range(30):
        ...
        log(log_file_path,"/epoch_"+str(j+1)+".pth")
        saved_models=torch.load(opt.load+"/epoch_"+str(j+1)+".pth")
        ...
        for i, batch in enumerate(tqdm(dloader_test()), 1):
            ...
            if i % 500 == 0:
                ...
                log(log_file_path,str('Episode [{}/{}]:\t\t\tAccuracy: {:.2f} ± {:.2f} % ({:.2f} %)'.format(i, opt.episode, avg, ci95, acc)))
